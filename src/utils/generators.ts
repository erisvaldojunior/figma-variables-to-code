...
`;
	...
	collections.forEach((collection) => {
		dartFile += `_${toPascalCase(collection.name)} get ${toCamelCase(collection.name)} => _${toPascalCase(collection.name)}();\n`;
	});
...
function generateDartCodeForCollection(
	...
	let dartCode = `final class _${collectionName} {\n`;
	// Create class constructor
	dartCode += `  const _${collectionName}();\n`;
	...
	Object.keys(groupedVariables).forEach((groupName) => {
		if (groupName === '__root__') {
			...
			dartCode += '\n';
			dartCode += `  static final _${groupNameCamelCase} = _${groupNamePascalCase}();\n`;
			dartCode += `  _${groupNamePascalCase} get ${groupNameCamelCase} => _${groupNameCamelCase};\n`;
		}
	});
	...
	Object.keys(groupedVariables).forEach((groupName) => {
		if (groupName !== '__root__') {
			dartCode += '\n';
			dartCode += `final class _${toPascalCase(groupName)} {\n`;
			dartCode += `  const _${toPascalCase(groupName)}();\n`;
			groupedVariables[groupName].forEach((variable) => {
				dartCode += generateDartCodeForVariable(variable, figmaVariables);
			});
			dartCode += '}\n';
